...
-- 2) Helpers FIRST (policies below depend on these)
...
-- 3) Membership RLS (after helpers exist)
...
-- 4) Backfill from existing profiles (idempotent; does not remove rows)
...
-- 5) Keep memberships in sync when profiles change (upsert only — never delete)
...
-- 6) Expand SELECT so org admins can see every org they manage (for switcher + billing)
